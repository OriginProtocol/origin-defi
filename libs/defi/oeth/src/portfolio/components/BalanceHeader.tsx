  ...
  return (
    ...
    >
      <CardContent {...cardProps} sx={{ ...cardProps?.sx, py: 3.5 }}>
        ...
        >
          <Typography>
            {intl.formatMessage({ defaultMessage: 'OETH Balance' })}
          </Typography>
          <InfoTooltip
            tooltipLabel={intl.formatMessage({
              defaultMessage: 'Connected wallet OETH balance',
            })}
          />
        </Stack>
        ...
        {isConnected ? (
          ...
          >
            {intl.formatNumber(
              +formatUnits(balance ?? 0n, tokens.mainnet.OETH.decimals),
              {
      ...
      <CardContent {...cardProps}>
        <Button size="large" variant={isConnected ? 'contained' : 'outlined'}>
          {intl.formatMessage({ defaultMessage: 'Get OETH' })}
        </Button>
      </CardContent>
    </Card>
  );
