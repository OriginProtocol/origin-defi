...
export const LineChart = <Datum,>({
  ...
  const { minX, maxX, minY, maxY } = getScaleDomains(data, series);
  ...
  const handlePointerMove = useCallback(
    ...
    [data, onHover, series, showTooltip, xScale],
  );
    ...
    >
      <svg width={width} height={height}>
        ...
        {activeIdx !== null && !hideHoverLine ? (
          <line
            x1={xScale(data[activeIdx]?.[series[0].xKey] as number)}
            x2={xScale(data[activeIdx]?.[series[0].xKey] as number)}
            y1={margins.top}
